...
program
  ...
  .action(async () => {
    createShareDwallet()
      .then(() => console.log("Create dwallet successfully"))
      .catch(console.log);
  });

program
  .command("send")
  .description("Send btc from BTC dwallet to other address")
...
program
  .command("btc_addr")
  .argument("dwalletID")
  .description("Get BTC address of Dwallet")
  .action(async (dWalletID) => {

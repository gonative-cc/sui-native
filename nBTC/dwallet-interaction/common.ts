...
import { UserShareEncryptionKeys, Curve, IkaTransaction, IkaClient, getNetworkConfig, SessionsManagerModule, CoordinatorInnerModule, prepareDKGSecondRoundAsync, publicKeyFromDWalletOutput, type IkaConfig, type DWallet, type DKGSecondRoundRequestInput, type DWalletCap, type ZeroTrustDWallet } from '@ika.xyz/sdk';
...
async function registerEncryptionKey(ikaClient: IkaClient, suiClient: SuiClient, userShares: UserShareEncryptionKeys) {
...
export async function createShareDwallet() {
	...
	const activeDWallet = await ikaClient.getDWalletInParticularState(dwalletID as string, 'Active');

...
export async function requestDKGFirstRound(
  ...
  const sessionIdentifierPreimage =
    startDKGFirstRoundEvents?.[0]?.session_identifier_preimage;

  return {
    dwalletID: dwalletID,
    sessionIdentifierPreimage: new Uint8Array(
      sessionIdentifierPreimage as number[],
    ),
  };
}
...
export async function createShareDwallet() {
  ...
  const awaitingKeyHolderSignatureDWallet =
  ...
  const { secretShare } =
    await keypairs.userShareEncryptionKeys.decryptUserShare(
      activeDWallet,
